...
library PiecewiseLogic {
    ...
    function integratePiecewiseFunction(int[] constants, uint[] parts, uint[] dividers, uint x) internal pure returns (int) {
        for ( uint i = 0; i < dividers.length; i++ ) {
            ...
            for ( uint j = pStart; j < dividers[i]; j++ ) {
                ...
                assembly {
                    ...
                    // sum += ((_x ** (power + 1)) * coef) / power
                    sum := add(sum, div(mul(exp(_x, power), coef), power))
                }
            }
