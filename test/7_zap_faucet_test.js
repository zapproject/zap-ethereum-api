...
contract('ZapFaucet', function (accounts) {
    ...
    beforeEach(async function deployContracts() {
        ...
        this.currentTest.zapFaucet = await deployZapFaucet(this.currentTest.zapToken.address, owner);
    });


    it("ZAP_FAUCET_1 - pay - Check payable function", async function () {
        const tDecimals = new BigNumber((await this.test.zapToken.decimals.call()).valueOf());
        ...
        await this.test.zapToken.allocate(this.test.zapFaucet.address, tokensForAllocate);
        await this.test.zapFaucet.sendTransaction({from: buyer, value: weiToSend});

        const buyerBalance = await this.test.zapToken.balanceOf.call(buyer);
    ...
    it("ZAP_FAUCET_2 - pay - Check that payable function will throw revert if msg.value <= 0", async function () {
        await this.test.zapToken.allocate(this.test.zapFaucet.address, 1000000 * Math.pow(10, 18));
    ...
    it("ZAP_FAUCET_3 - withdrawZap() - Check that withdrawZap() function is working", async function () {
        ...
        const ownerBalance = await this.test.zapToken.balanceOf.call(owner);
