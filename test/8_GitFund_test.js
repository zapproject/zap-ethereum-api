...
contract('GitFund', function (accounts) {
  ...
  let btcContest = await BTCcontest.new(this.test.coord.address,factory.address, startPrice, upEndpoint, downEndpoint);
        ...
        await reserveToken.allocate(owner, tokensAllocation);
        await reserveToken.allocate(participant1, tokensAllocation, {from: owner});
        await reserveToken.allocate(participant2, tokensAllocation, {from: owner});
        await reserveToken.allocate(btcContest.address, tokensAllocation, {from: owner});
        await reserveToken.approve(factory.address, tokensAllocation, {from: owner});
        await reserveToken.approve(factory.address, tokensAllocation, {from: participant1});
        await reserveToken.approve(factory.address, tokensAllocation, {from: participant2});
