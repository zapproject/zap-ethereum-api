...
contract('TokenDotFactory', function (accounts) {
    ...
    beforeEach(async function deployContracts() {
        ...
        this.currentTest.custom_token = await ZapToken.new();
				this.currentTest.testToken = await ZapToken.new();
				this.currentTest.testToken.address = "0x0";// = await ZapToken.new();
    ...
    it("TOKEN_DOT_FACTORY_3 - initializeCurve() - Check curve initialization", async function () {
				let token = '0x0';
				let factory = await TokenDotFactory.new(this.test.coord.address, this.test.tokenFactory.address, publicKey, title);
        let tx = await factory.initializeCurve(specifier, "sbl", piecewiseFunction, this.test.testToken.address);
    ...
    it("TOKEN_DOT_FACTORY_4 - initializeCurve() - Exception thrown if curve specifier already exists", async function () {
        ...
        await expect(factory.initializeCurve(specifier, "sbl", piecewiseFunction, this.test.testToken.address)).to.be.eventually.rejectedWith(EVMRevert);
    });
